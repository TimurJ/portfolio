...
| `--img`         | `grayscale(1) contrast(1.05)` | same                    | Photography filter           |
...
- The design persists the user's choice in localStorage under `tj-portfolio-theme` and animates the flip with a ~260ms color transition (transition + toggle are page-build work, not part of the token layer).
...
## Deferred to the page build

- All page markup (header/hero/experience/blog/contact/footer), behavior JS (theme toggle, active-nav sync, smooth scroll, pill balancing, read-more), the portrait asset (`assets/portrait-bw.png`), and container-query responsive rules.
