...
- Article pages have no page in the design — **added**. The header reuses the section intro's ladder for the title (`clamp(28px, 4.4vh, 50px)`, 38px / 28px on the laptop / phone tiers) and `h2`s reuse the blog row's (`clamp(20px, 2vw, 30px)`, 22px / 20px); the body is a reading size the design never needed, 17px at 1.65 line-height (16px on the laptop tier, `--text-body` 15px on the phone tier), with code at 13.5px / 12.5px. Code colours come from Shiki's `github-light-default` / `github-dark-default`, the one place the palette is not the design's; the contrast reasoning is in [architecture.md](architecture.md).
