...
The invariant holds for the chrome as well as the sections — `Header` and `ThemeToggle` earn no carve-out, since two policies would be the real smell. It has exactly one exception, and it is about the utility rather than the component: a responsive variant may stay in markup when the utility it modifies is a bundle with no single-declaration equivalent. `sr-only` is the only such utility in play (eight declarations; hand-copying them into a scoped block is worse, and `@apply` there would need a `@reference` import of `global.css`), and `ThemeToggle`'s phone-tier label is its only use. Everything else — a gap, a padding, a font size, a `flex-wrap` — is one declaration and moves.
